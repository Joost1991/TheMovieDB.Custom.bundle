...
TMDB_TV_EPISODE_GROUPS = '/tv/%s/episode_groups?language=%s'
TMDB_TV_EPISODE_GROUP = '/tv/episode_group/%s?language=%s'
...
def GetImdbId(tmdb_id, lang='en'):
    ...
    if isinstance(tmdb_dict, dict) and 'imdb_id' in tmdb_dict and tmdb_dict['imdb_id'] and RE_IMDB_ID.search(
            tmdb_dict['imdb_id']):
...
def GetTMDBJSON(url, cache_time=CACHE_1MONTH):
    ...
    try:
        tmdb_dict = JSON.ObjectFromURL(TMDB_BASE_URL % String.Quote(url, True), sleep=2.0,
                                       headers={'Accept': 'application/json'}, cacheTime=cache_time)
    except:
    ...
    if media.primary_metadata is not None and RE_IMDB_ID.search(media.primary_metadata.id):
        ...
        # If this is an automatic search and The Movie Database agent is used as a primary agent.
            ...
            tmdb_dict = GetTMDBJSON(
                url=TMDB_MOVIE_SEARCH % (String.Quote(stripped_name, True), year, lang, include_adult))
            if media.name != stripped_name and (tmdb_dict is None or len(tmdb_dict['results']) == 0):
                ...
                tmdb_dict = GetTMDBJSON(
                    url=TMDB_MOVIE_SEARCH % (String.Quote(media.name, True), year, lang, include_adult))
...
def PerformTMDbMovieUpdate(metadata_id, lang, existing_metadata):
                ...
                if country['iso_3166_1'] == Locale.CountryCodes.MatchToCode(c):
                    ...
                    if 'certification' in country and country['certification'] != '':
                        if Locale.CountryCodes.MatchToCode(c) == 'US':
                            ...
                            metadata['content_rating'] = '%s/%s' % (
                                Locale.CountryCodes.MatchToCode(c).lower(), country['certification'])
    ...
    # Runtime.
    try:
        metadata['duration'] = int(tmdb_dict['runtime']) * 60 * 1000
    except:
        pass
    ...
    if Prefs['collections'] and isinstance(tmdb_dict.get('belongs_to_collection', None), dict):
        metadata['collections'].append(
            (tmdb_dict['belongs_to_collection'].get('name') or '').replace(' Collection', ''))
                ...
                try:
                    if i > ARTWORK_ITEM_LIMIT:
                        ...
                        if poster_url not in existing_metadata.posters:
                            try:
                                metadata['posters'][poster_url] = (thumb_url, i + 1)
                            except:
                                pass
                except:
                    pass
                ...
                try:
                    if i > ARTWORK_ITEM_LIMIT:
                        ...
                        if backdrop_url not in existing_metadata.art:
                            try:
                                metadata['art'][backdrop_url] = (thumb_url, i + 1)
                            except:
                                pass
                except:
                    pass
...
class TMDbAgent(Agent.Movies):
    name = 'The Movie Database (Custom)'
    languages = LANGUAGES
...
class TMDbAgent(Agent.TV_Shows):
    ...
    def search(self, results, media, lang, manual):
        ...
        if not manual:
            ...
            if (' (US)' in filename or ' (UK)' in filename) and (
                    media.show.endswith(' Us') or media.show.endswith(' Uk')):
                media_show = media.show.rsplit(' ', 1)[0]

        Log('Search query is ' + media_show)
        id_series = find_id(r'(?<=tmdb)\d+|(?<=tmdb-)\d+|(?<=tmdb )\d+|(?<=tmdb_)\d+|(?<=TheMovieDB::TV_)\d+',
                            media_show)

        if manual and id_series is not None:
            Log('Searching Series \"' + xstr(media.show) + '\" By TMDB ID \'' + id_series + '\'')
            tmdb_dict = GetTMDBJSON(url=TMDB_TV % (id_series, lang))
            tmdb_episode_group_dict = GetTMDBJSON(url=TMDB_TV_EPISODE_GROUPS % (id_series, lang))

            for episode_group in tmdb_episode_group_dict["results"]:
                results.Append(MetadataSearchResult(
                    id=str(tmdb_dict['id']) + "-" + str(episode_group['id']),
                    name=tmdb_dict['name'] + " - " + episode_group['name'],
                    score=250,
                    lang=lang
                ))
    ...
    def update(self, metadata, media, lang):
        ...
        Log("This is updating now with following values:")
        Log(metadata.id)

        episode_groups = False
        ...
        if "-" in metadata.id:
            split = metadata.id.split("-")
            id = split[0]
            episode_group_id = split[1]
            episode_groups = True
            Log(id)
            Log(episode_group_id)

        tmdb_dict = GetTMDBJSON(url=TMDB_TV % (metadata.id, lang))

        if not isinstance(tmdb_dict, dict) or 'overview' not in tmdb_dict or tmdb_dict['overview'] is None or tmdb_dict[
            'overview'] == "":
            # Retry the query with no language specified if we didn't get anything from the initial request.
        ...
        # Runtime.
        try:
            metadata.duration = int(tmdb_dict['episode_run_time'][0]) * 60 * 1000
        except:
            pass
                ...
                if country['iso_3166_1'] == Locale.CountryCodes.MatchToCode(c):
                    ...
                    if 'rating' in country and country['rating'] != '':
                        if Locale.CountryCodes.MatchToCode(c) == 'US':
                            ...
                            metadata.content_rating = '%s/%s' % (
                                Locale.CountryCodes.MatchToCode(c).lower(), country['rating'])
                    break
        except:
            pass
        ...
        def UpdateEpisodes():
            ...
            for s in media.seasons:
                ...
                if episode_groups:
                    tmdb_season_dict = GetTMDBJSON(url=TMDB_TV_EPISODE_GROUP % (episode_group_id, lang))

                    for group in tmdb_season_dict["groups"]:
                        Log(group["order"])
                        Log(s)
                        if int(group["order"]) == int(s):
                            Log("Found the right season in episode group")
                            tmdb_episodes_dict = group["episodes"]
                            break
                ...
                for e in media.seasons[s].episodes:
                    ...
                    @task
                    def UpdateEpisode(episode=episode, tmdb_season_dict=tmdb_season_dict,
                                      tmdb_episodes_dict=tmdb_episodes_dict, s=s, e=int(e)):

                        if not episode_groups or not isinstance(tmdb_season_dict, dict) or tmdb_episodes_dict is None:
                            Log("No instance of tmdb_episodes_dict")
                            return None

                        # Rating.
                        votes = tmdb_episodes_dict[e - 1]['vote_count']
                        rating = tmdb_episodes_dict[e - 1]['vote_average']
                        if votes > 3:
                            episode.rating = rating

                        # Name of the episode.
                        episode.title = tmdb_episodes_dict[e - 1]['name']

                        # Summary.
                        episode.summary = tmdb_episodes_dict[e - 1]['overview']

                        # Air date.
                        try:
                            episode.originally_available_at = Datetime.ParseDate(
                                tmdb_episodes_dict[e - 1]['air_date']).date()
                        except:
                            pass
                        ...
                        Log("Updated episode S" + str(s) + "E" + str(e) + " successfully")
                    ...
                    if not episode_groups:
                        ...
                        if 'poster_path' in tmdb_season_dict and tmdb_season_dict['poster_path']:
                            poster_url = config_dict['images']['base_url'] + 'original' + tmdb_season_dict[
                                'poster_path']
                            thumb_url = config_dict['images']['base_url'] + 'w154' + tmdb_season_dict['poster_path']
                            valid_names.append(poster_url)
                        ...
                        for e in media.seasons[s].episodes:
                            ...
                            def UpdateEpisode(episode=episode, s=s, e=e):
                                ...
                                try:
                                    episode.originally_available_at = Datetime.ParseDate(
                                        tmdb_episode_dict['air_date']).date()
                                except:
                                    pass
                                ...
                                if 'crew' in tmdb_episode_dict['credits']:
                                    ...
                                    for member in tmdb_episode_dict['credits']['crew']:
                                        if member['job'] == 'Director':
                                            ...
                                            director.name = member['name']
                                            if member['profile_path'] is not None:
                                                director.photo = config_dict['images']['base_url'] + 'original' + \
                                                                 member['profile_path']
                                        elif member['job'] in ('Writer', 'Screenplay'):
                                            writer = episode.writers.new()
                                            writer.name = member['name']
                                            if member['profile_path'] is not None:
                                                writer.photo = config_dict['images']['base_url'] + 'original' + \
                                                               member['profile_path']
                                        elif member['job'] == 'Producer':
                                            producer = episode.producers.new()
                                            producer.name = member['name']
                                            if member['profile_path'] is not None:
                                                producer.photo = config_dict['images']['base_url'] + 'original' + \
                                                                 member['profile_path']
                                ...
                                # Check if there are stills present and then loop through them
                                if tmdb_episode_dict['images']['stills']:
                                    max_average = max([(lambda p: p['vote_average'] or 5)(p) for p in
                                                       tmdb_episode_dict['images']['stills']])
                                    max_count = max([(lambda p: p['vote_count'])(p) for p in
                                                     tmdb_episode_dict['images']['stills']]) or 1
                                    ...
                                    for i, still in enumerate(
                                            sorted(tmdb_episode_dict['images']['stills'], key=lambda k: k['score'],
                                                   reverse=True)):
                                        if i > ARTWORK_ITEM_LIMIT:
                                            break
                                        else:
                                            still_url = config_dict['images']['base_url'] + 'original' + still[
                                                'file_path']
                                            thumb_url = config_dict['images']['base_url'] + 'w154' + still[
                                                'file_path']
                                            valid_names.append(still_url)

                                            if still_url not in episode.thumbs:
                                                try:
                                                    episode.thumbs[still_url] = Proxy.Preview(
                                                        HTTP.Request(thumb_url).content, sort_order=i + 1)
                                                except:
                                                    pass
